...
class GraphPane():
	...
	def updateGraph(self, stage=None, ranges=False, importing=False):
		...
		"""
		# For the first time, or after 'ratio' is called:
		if importing:
			# if this is the first time, draw all elements on the graph
			self.graph.initialiseGraph()
		else:
			# otherwise, just update the elements
...
class GraphWindow(QWidget):
	...
	def updateFocus(self):
		...
		if self.focusStage in ['rawdata', 'despiked']:
			for graph in self.graphs:
